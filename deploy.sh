...
# =============================================================================
# deploy.sh
# Deploys the Expense Management application to Azure WITHOUT GenAI services.
#
# Prerequisites:
...
#   ADMIN_OBJECT_ID="<your-entra-object-id>" ADMIN_LOGIN="<your-email@domain.com>" ./deploy.sh
#
# Or set variables at the top of this file and just run:
#   ./deploy.sh
# =============================================================================
...
ADMIN_OBJECT_ID="${ADMIN_OBJECT_ID:-}"    # Your Entra ID Object ID
ADMIN_LOGIN="${ADMIN_LOGIN:-}"             # Your UPN e.g. name@domain.com
# ──────────────────────────────────────────────────────────────────────────────

echo ""
echo "═══════════════════════════════════════════════════════════════"
echo "  Expense Management System – Deploy (no GenAI)"
echo "═══════════════════════════════════════════════════════════════"
echo ""
...
if [ -z "$ADMIN_OBJECT_ID" ] || [ -z "$ADMIN_LOGIN" ]; then
    ...
    echo "   Run: ADMIN_OBJECT_ID=\"<id>\" ADMIN_LOGIN=\"<email>\" ./deploy.sh"
    exit 1
fi
...
echo ""
echo "🏗️  Step 2: Deploying Azure infrastructure (App Service + SQL)..."
...
echo ""
echo "⚙️  Step 3: Configuring App Service settings..."
...
if [ "$MY_IP" != "0.0.0.0" ]; then
    az sql server firewall-rule create \
        ...
        --output none
    echo "   ✓ Added deployment IP: $MY_IP"
fi
...
# Update Python script with actual server/database
...
# ── Step 10: Build and package the application ───────────────────────────────
echo ""
...
content = re.sub(
    r'REPLACE_SQL_SERVER',
    '${SQL_SERVER_NAME}',
    content
)
content = re.sub(
    r'REPLACE_MANAGED_IDENTITY_CLIENT_ID',
...
echo "   ✓ Application packaged as app.zip"
...
echo "  💬 AI Chat (demo)  : ${APP_SERVICE_URL}/Chat"
echo ""
echo "  ⚠️  NOTE: Navigate to /Index (not the root URL) to view the app."
echo "  💡 To enable AI features, run: ./deploy-with-chat.sh"
echo ""
